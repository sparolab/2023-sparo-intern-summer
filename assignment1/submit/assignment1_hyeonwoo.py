...
C02L = np.concatenate((R, np.reshape(t, (3, 1))), axis=1)
...
C02C2 = np.concatenate((R_02,np.reshape(T_02,(3,1))),axis=1)
...
## Each column represent cam2's 3D points
## The z-axis of the camera coordinate is in the direction of the optical part, so the back part is removed

ptcloud_cam2 = ptcloud_cam2[:,ptcloud[2]>0]

## 3. Normalization
## Project in 2D via projection matrix
## By dividing by the z, the x&y values are placed in the same plain

ptcloud_image = np.matmul(P,ptcloud_cam2)
ptcloud_image = ptcloud_image / ptcloud_image[2,:]

## 4. 이미지 바깥에 있는 Point (Outlier) 제거

inner = (ptcloud_image[0] >=0)&(ptcloud_image[0]<w)&(ptcloud_image[1]>=0)&(ptcloud_image[1]<h)
ptcloud_cam2=ptcloud_cam2[:,inner]
ptclout_image=ptcloud_image[:,inner]
