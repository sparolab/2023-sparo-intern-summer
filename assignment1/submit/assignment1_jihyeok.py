...
C02C2 = np.insert(R_02, 3, values=t, axis=1)
...
pt2cam = np.dot(C22L,ptcloud) # pointcloud를 cam2좌표계로 변환

pt2cam = pt2cam[:,pt2cam[2,:] > 0]

pt2img = np.matmul(P, pt2cam)
pt2img = pt2img[:2] / pt2img[2] # (u,v) = (x/z, y/z)

mask = (pt2img[0] >= 0)&(pt2img[0] < w)&(pt2img[1] >= 0)&(pt2img[1] < h)
pt2img = pt2img[:,mask]
pt2cam = pt2cam[:,mask]
print(pt2cam)
u = pt2img[0]
