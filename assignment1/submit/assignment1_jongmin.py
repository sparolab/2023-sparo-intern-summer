...
## =================================================================
C22L = np.dot(np.linalg.inv(C02C2),C02L)
## =================================================================
...
## 3. Normalization
ptcloud_c2[0,:] /= ptcloud_c2[2,:]
ptcloud_c2[1,:] /= ptcloud_c2[2,:]
z = ptcloud_c2[2, :]

## 4. 이미지 바깥에 있는 Point (Outlier) 제거

# 이미지 좌표로 변환
x1 = K[0, 0] * ptcloud_c2[0,:] + K[0, 2]
y1 = K[1, 1] * ptcloud_c2[1,:] + K[1, 2]

# Stack x1 and y1 to create a 2D point array, and reshape it for undistortPoints
xy_points = np.stack((x1, y1), axis=-1)

# Correct for distortion
xy_points_undistorted = cv2.undistortPoints(xy_points, cameraMatrix=K, distCoeffs=D, P=P)

# The undistorted points are now in homogeneous coordinates, so convert back to 2D
u = xy_points_undistorted[:,0,0]
v = xy_points_undistorted[:,0,1]

reduce_img = (u >= 0) & (u < w) & (v >= 0) & (v < h)
u = u[reduce_img]
v = v[reduce_img]
z = z[reduce_img]
