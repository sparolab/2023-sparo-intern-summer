## =================================================================
## Situation: LiDAR Point를 Camera2에 투영하기
## Intrinsic parameter: C2
## Extrinsic parameter: C2->C0, C0->LiDAR
## =================================================================
...
## 3. Normalization: 각 포인트의 좌표를 해당 포인트의 깊이로 나누어주는 것
## 4. 이미지 바깥에 있는 Point (Outlier) 제거

## 1
ptcloud_cam = np.matmul(C22L,ptcloud)
## 2
ptcloud_cam = ptcloud_cam[:, ptcloud_cam[2] > 0]
## 3
ptcloud_image = np.matmul(P, ptcloud_cam) #투영
ptcloud_image /= ptcloud_image[2] #정규화
## 4
mask = (ptcloud_image[0] >= 0) & (ptcloud_image[0] < w) & (ptcloud_image[1] >= 0) & (ptcloud_image[1] < h) #mask 생성
ptcloud_image = ptcloud_image[:, mask] #mask에 해당하는 열만 포해, 이미지 경계 내에 있는 것만 보이도록 설정
ptcloud_cam = ptcloud_cam[:, mask]  

u = ptcloud_image[0] #이미지 좌표
v = ptcloud_image[1] #이미지 좌표
z = ptcloud_cam[2] #카메라 좌표
